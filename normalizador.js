window.AuditoriaNormalizador = (() => {
  ...
  function normalize(doc,type){const sh=bestSheet(doc,type),hh=headers(sh.rows),periods=Object.keys(hh).sort(),matched={},unmapped=[];sh.rows.forEach(row=>{const text=label(row,type);if(!text||badSheet.test(norm(text)))return;const m=match(text,type);if(!m)return;const values={};periods.forEach(y=>values[y]=number(row[hh[y]]));if(!Object.values(values).some(v=>v!==null))return;if(!matched[m.key])matched[m.key]={label:text,alias:m.alias,score:m.score,values};else periods.forEach(y=>{if(matched[m.key].values[y]===null&&values[y]!==null)matched[m.key].values[y]=values[y];});});return{name:doc.name,sheet:sh.name,periods,matched,unmapped};}
  function normalizeCombined(doc){const sh=bestSheet(doc,'balance'),hh=headers(sh.rows),periods=Object.keys(hh).sort(),out={balance:{name:doc.name,sheet:sh.name,periods,matched:{},unmapped:[]},resultados:{name:doc.name,sheet:sh.name,periods,matched:{},unmapped:[]}};let section=null;sh.rows.forEach(row=>{const txt=rowText(row);if(/^1\.\s*balance general/.test(txt)||txt==='balance general'){section='balance';return;}if(/^2\.\s*estado de resultados/.test(txt)||txt==='estado de resultados'){section='resultados';return;}if(!section)return;const text=label(row,section);if(!text||badSheet.test(norm(text)))return;const m=match(text,section);if(!m)return;const values={};periods.forEach(y=>values[y]=number(row[hh[y]]));if(!Object.values(values).some(v=>v!==null))return;if(!out[section].matched[m.key])out[section].matched[m.key]={label:text,alias:m.alias,score:m.score,values};else periods.forEach(y=>{if(out[section].matched[m.key].values[y]===null&&values[y]!==null)out[section].matched[m.key].values[y]=values[y];});});return out;}

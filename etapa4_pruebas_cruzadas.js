...
(function(){
  ...
  function status(s){return s==='CONCILIADO'?'okcell':s==='SIN DATOS'||s==='PARA INDAGAR'?'warncell':'badcell';}
  ...
  function cashFlow(d,f,i,p){
    ...
    const calc=cash-cash0,diff=calc-flow;return row('5. Caja ↔ Flujo de fondos','Caja final − caja inicial = variación neta de caja.',`Caja inicial ${fmt(cash0)} · Caja final ${fmt(cash)} · Variación calculada ${fmt(calc)} · Flujo ${fmt(flow)} · Diferencia ${fmt(diff)}`,Math.abs(diff)<=1?'CONCILIADO':'INCONSISTENCIA');
  }
  ...
  function render(d,f){
    ...
    html+='<div class="audit-current"><b>CRITERIO</b><br><b>CONCILIADO</b> = la relación matemática evaluada cuadra. <b>CORRELACIÓN</b> = existe relación económica útil, pero no demuestra por sí sola un error o una causa. <b>PARA INDAGAR</b> = la relación presenta una señal que requiere explicación documental o gerencial. <b>INCONSISTENCIA</b> = la relación matemática evaluada no cuadra. <b>SIN DATOS</b> = faltan importes suficientes.</div></div></section>';
    const old=$('pruebasCruzadasSection');if(old)old.remove();$('auditoria').insertAdjacentHTML('beforeend',html);
  }
  async function run(){try{const rows=await readRows();const d=extract(rows,aliases,9,120);const f=extract(rows,flujoAliases,120,rows.length);render(d,f);}catch(e){const old=$('pruebasCruzadasSection');if(old)old.remove();$('auditoria').insertAdjacentHTML('beforeend',`<section class="card" id="pruebasCruzadasSection"><h2>6. PRUEBAS CRUZADAS</h2><div class="body"><div class="alert crit"><b>ERROR EN PRUEBAS CRUZADAS:</b> ${esc(e.message)}</div></div></section>`);}}

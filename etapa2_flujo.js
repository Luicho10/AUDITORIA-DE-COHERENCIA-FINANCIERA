/* ETAPA 2 - VALIDACION DEL FLUJO DE FONDOS COMPARATIVO - V3 */
(function(){
  ...
  function row(label,checks,resultOverride=null){const ok=checks.every(x=>x.ok);const result=resultOverride||(ok?'CONCILIADO':'INCONSISTENCIA');const cls=result==='CONCILIADO'?'okcell':result==='OBSERVACIÓN'||result==='PARA INDAGAR'?'warncell':'badcell';return `<tr><td><b>${esc(label)}</b></td><td>${checks.map(x=>esc(x.detail)).join('<br>')}</td><td class="${cls}"><b>${esc(result)}</b></td></tr>`;}
  ...
  function render(x){
    ...
    for(let i=1;i<3;i++){
      ...
      /* El desequilibrio inicial se calcula directamente contra la ecuación patrimonial:
         Activo - (Pasivo + Patrimonio). No se utiliza totalP, porque puede representar
         únicamente una de las agrupaciones del balance según el formato de entrada. */
      const openingEquation=B.totalA[i-1]-(B.totalPC[i-1]+B.totalPNC[i-1]+B.equity[i-1]);
      const openingEquationAlt=B.totalA[i-1]-B.totalP[i-1];
      const openingImbalance=Math.abs(openingEquation)>1?openingEquation:openingEquationAlt;
      const cashDiff=F.netCash[i]-(B.cash[i]-B.cash[i-1]);
      const cashInherited=Math.abs(cashDiff-openingImbalance)<=1&&Math.abs(openingImbalance)>1;
      const cashCheck=status(F.netCash[i],B.cash[i]-B.cash[i-1]);
      checks.push(Object.assign(cashCheck,{detail:`Variación neta de caja: ${fmt(F.netCash[i])} · Caja ${y} − Caja ${prev}: ${fmt(B.cash[i]-B.cash[i-1])} · Diferencia: ${fmt(cashDiff)}`}));
      ...
      const payableBalance=(B.providers[i]-B.providers[i-1])+(B.clientAdv[i]-B.clientAdv[i-1])+(B.creditors[i]-B.creditors[i-1])+(B.otherPC[i]-B.otherPC[i-1]);
      const payableDiff=F.varProviders[i]-payableBalance;
      checks.push(Object.assign(status(F.varProviders[i],payableBalance),{detail:`Cuentas a pagar utilizadas por el flujo: ${fmt(F.varProviders[i])} · Proveedores + Anticipos de clientes + Acreedores + otras cuentas a pagar: ${fmt(payableBalance)} · Diferencia: ${fmt(payableDiff)}`}));
      const deltaPPE=B.ppe[i]-B.ppe[i-1],expectedPPE=-deltaPPE;
      ...
      const cashRow=cashInherited?observation('1. Caja ↔ Variación neta de caja',`Variación del flujo ${fmt(F.netCash[i])} · Variación según Balance ${fmt(B.cash[i]-B.cash[i-1])} · Diferencia ${fmt(cashDiff)}. La diferencia coincide con el desequilibrio patrimonial existente al cierre de ${prev}: Activo − (Pasivo + Patrimonio) = ${fmt(openingImbalance)}. Se considera una diferencia heredada del saldo inicial y no una inconsistencia generada durante ${y}.`):row('1. Caja ↔ Variación neta de caja',[checks[0]]);
      const payableMaterial=payableBalance!==0&&Math.abs(payableDiff)/Math.max(Math.abs(payableBalance),Math.abs(F.varProviders[i]),1)>0.05;
      const payableResult=Math.abs(payableDiff)<=1?'CONCILIADO':'PARA INDAGAR';
      const payableDetail=`La diferencia representa ${payableMaterial?'un desvío que requiere explicación':'un desvío menor'} frente al movimiento de cuentas a pagar. Puede obedecer a pagos, compensaciones, reclasificaciones, anticipos, otras cuentas o criterios de presentación del flujo; no se considera por sí sola una inconsistencia contable.`;
      checks[4].detail+=` ${payableDetail}`;
      const payableRow=row('5. Cuentas a pagar ↔ Variación del flujo',[checks[4]],payableResult);
      const resultRows=`${cashRow}${row('2. Resultado utilizado por el Flujo ↔ Resultado antes de impuesto',[checks[1]])}${observation('2A. Resultado Neto contable ↔ valor rotulado como Resultado Neto del Flujo',obs.join(' ')||'El flujo utiliza Resultado antes de impuesto como punto de partida. La diferencia frente al Resultado Neto contable corresponde al tratamiento del impuesto y se presenta como diferencia metodológica.')}${row('3. Clientes ↔ Variación de cuentas a cobrar',[checks[2]])}${row('4. Inventario + Anticipos ↔ Variación',[checks[3]])}${payableRow}${row('6. Activo Fijo ↔ Inversiones/variación',[checks[5]])}${row('7. Capital ↔ Aportes/variación',[checks[6]])}${row('8. Flujo operativo + no operativo ↔ Variación de caja',[checks[7]])}`;
      out.push(`<div class="alert ${good?'ok':'warn'}"><strong>${y} — VALIDACIÓN DEL FLUJO</strong><div>Comparación contra ${prev} y contra los Estados Financieros.</div></div><table class="table"><thead><tr><th>Prueba</th><th>Conciliación</th><th>Resultado</th></tr></thead><tbody>${resultRows}</tbody></table>`);
    }
    $('auditoria').innerHTML=`<section class="card"><h2>4. VALIDACIÓN DEL FLUJO DE FONDOS COMPARATIVO</h2><div class="body"><div class="audit-current"><b>CONTROL ESPECÍFICO</b><br>Se validan las relaciones matemáticas del flujo contra Balance y Estado de Resultados. Las diferencias heredadas de un saldo inicial o derivadas de criterios de presentación se muestran como observaciones o señales para indagar, no como errores automáticos.</div>${out.join('')}<div class="audit-current"><b>CRITERIO</b><br><b>CONCILIADO</b> = la relación evaluada cuadra. <b>PARA INDAGAR</b> = existe un desvío que requiere explicación documental o gerencial, pero no demuestra por sí solo un error contable. <b>INCONSISTENCIA</b> = la relación contable evaluada no cuadra y existe evidencia suficiente de que no puede explicarse por una diferencia heredada o metodológica. <b>OBSERVACIÓN</b> = existe una diferencia de presentación, metodología o saldo inicial que requiere interpretación.</div></div></section>`;

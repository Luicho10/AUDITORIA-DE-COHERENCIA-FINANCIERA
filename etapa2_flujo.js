/* ETAPA 2 - VALIDACION DEL FLUJO DE FONDOS COMPARATIVO - V2 */
(function(){
  ...
  function render(x){
    ...
    for(let i=1;i<3;i++){
      ...
      const openingImbalance=B.totalA[i-1]-B.totalP[i-1];
      ...
      const cashRow=cashInherited?observation('1. Caja ↔ Variación neta de caja',`Variación del flujo ${fmt(F.netCash[i])} · Variación según Balance ${fmt(B.cash[i]-B.cash[i-1])} · Diferencia ${fmt(cashDiff)}. La diferencia coincide con el desequilibrio patrimonial del período inicial (${prev}): Activo − Pasivo − Patrimonio = ${fmt(openingImbalance)}. Se considera diferencia heredada del saldo inicial y no una inconsistencia generada durante ${y}.`):row('1. Caja ↔ Variación neta de caja',[checks[0]]);
      const payableStatus=Math.abs(payableDiff)<=1?'CONCILIADO':Math.abs(payableDiff)>1000000000?'INCONSISTENCIA':'INCONSISTENCIA';
      const payableRow=payableStatus==='CONCILIADO'?row('5. Cuentas a pagar ↔ Variación del flujo',[checks[4]]):row('5. Cuentas a pagar ↔ Variación del flujo',[checks[4]]);
      out.push(`<div class="alert ${good?'ok':'warn'}"><strong>${y} — VALIDACIÓN DEL FLUJO</strong><div>Comparación contra ${prev} y contra los Estados Financieros.</div></div><table class="table"><thead><tr><th>Prueba</th><th>Conciliación</th><th>Resultado</th></tr></thead><tbody>${cashRow}${row('2. Resultado utilizado por el Flujo ↔ Resultado antes de impuesto',[checks[1]])}${observation('2A. Resultado Neto contable ↔ valor rotulado como Resultado Neto del Flujo',obs.join(' ')||'El flujo utiliza Resultado antes de impuesto como punto de partida. La diferencia frente al Resultado Neto contable corresponde al tratamiento del impuesto y se presenta como diferencia metodológica.')}${row('3. Clientes ↔ Variación de cuentas a cobrar',[checks[2]])}${row('4. Inventario + Anticipos ↔ Variación',[checks[3]])}${payableRow}${row('6. Activo Fijo ↔ Inversiones/variación',[checks[5]])}${row('7. Capital ↔ Aportes/variación',[checks[6]])}${row('8. Flujo operativo + no operativo ↔ Variación de caja',[checks[7]])}</tbody></table>`);
    }
    $('auditoria').innerHTML=`<section class="card"><h2>4. VALIDACIÓN DEL FLUJO DE FONDOS COMPARATIVO</h2><div class="body"><div class="audit-current"><b>CONTROL ESPECÍFICO</b><br>Se validan las relaciones matemáticas del flujo contra Balance y Estado de Resultados. Una diferencia coincidente con un desequilibrio del período inicial se presenta como observación heredada y no como error propio del flujo.</div>${out.join('')}<div class="audit-current"><b>CRITERIO</b><br><b>CONCILIADO</b> = la relación evaluada cuadra. <b>INCONSISTENCIA</b> = la relación contable evaluada no cuadra y no puede explicarse por una diferencia heredada o metodológica. <b>OBSERVACIÓN</b> = existe una diferencia de presentación, metodología o saldo inicial que requiere interpretación.</div></div></section>`;
